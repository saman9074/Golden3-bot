...
local function run(msg, matches)
	if permissions(msg.from.id, msg.to.id, "lang_install") then
		...
		set_text(LANG, 'EnglishL', 'English is now allowed in this supergroup.')
		set_text(LANG, 'noEnglishT', 'English is not allowed in this chat.')
		set_text(LANG, 'noEnglishL', 'English is not allowed in this supergroup.')

		set_text(LANG, 'SwearingT', 'Bad words is now allowed in this chat.')
		set_text(LANG, 'SwearingL', 'Bad words is now allowed in this supergroup.')
		set_text(LANG, 'noSwearingT', 'Bad words is not allowed in this chat.')
		set_text(LANG, 'noSwearingL', 'Bad words is not allowed in this supergroup.')
	    ...
	    set_text(LANG, 'settings:4', '#settings english enable/disabl: when disabled, all messages with English will be cleared.')
	    set_text(LANG, 'settings:5', '#settings bdw enable/disabl: when disabled, all messages with Bad Worlds will be cleared.')
	    set_text(LANG, 'settings:6', '#settings bots enable/disable: when disabled, if someone adds a bot, it will be kicked.')
	    set_text(LANG, 'settings:7', '#settings gifs enable/disable: when disabled, all gifs will be cleared.')
	    set_text(LANG, 'settings:8', '#settings photos enable/disable: when disabled, all photos will be cleared.')
	    set_text(LANG, 'settings:9', '#settings audios enable/disable: when disabled, all audios will be cleared.')
	    set_text(LANG, 'settings:10', '#settings kickme enable/disable: when disabled, people can kick out itself.')
	    set_text(LANG, 'settings:11', '#settings spam enable/disable: when disabled, all spam links will be cleared.')
	    set_text(LANG, 'settings:12', '#settings setphoto enable/disable: when enabled, if a user changes the group photo, the bot will revert to the saved photo.')
	    set_text(LANG, 'settings:13', '#settings setname enable/disable: when enabled, if a user changes the group name, the bot will revert to the saved name.')
	    set_text(LANG, 'settings:14', '#settings lockmember enable/disable: when enabled, the bot will kick all people that enters to the group.')
	    set_text(LANG, 'settings:15', '#settings floodtime <secs>: set the time that bot uses to check flood.')
	    set_text(LANG, 'settings:16', '#settings maxflood <msgs>: set the maximum messages in a floodtime to be considered as flood.')
	    set_text(LANG, 'settings:17', '#setname <group title>: the bot will change group title.')
	    set_text(LANG, 'settings:18', '#setphoto <then send photo>: the bot will change group photo.')
	    set_text(LANG, 'settings:19', '#lang <language (en, es...)>: it changes the language of the bot.')
	    set_text(LANG, 'settings:20', '#setlink <link>: saves the link of the group.')
	    set_text(LANG, 'settings:21', '#link: to get the link of the group.')
